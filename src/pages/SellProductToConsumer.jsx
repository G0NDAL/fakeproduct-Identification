...
const SellProductToConsumer = () => {
  const { supplyChain, productRegistry, account, isCorrectNetwork } = useWeb3();
  ...
  const handleFileChange = async (e) => {
    ...
    const tmp = new Html5Qrcode('temp-upload-qr-div-consumer');
    try {
  ...
  const handleSell = async (e) => {
    e.preventDefault();
    setError('');
    if (!serialNumber || !consumerCode) { setError('Both fields are required'); return; }
    ...
    setTxStatus('pending');
    try {
      const tx = await supplyChain.sellToConsumer(serialNumber.trim(), consumerCode.trim());
      setTxHash(tx.hash);
      await tx.wait();
      setTxStatus('success');

      // Fetch product info for invoice + history
      const product  = await productRegistry.getProduct(serialNumber.trim());
      const history  = await supplyChain.getProductHistory(serialNumber.trim());
      const statusMap = ['Manufactured', 'Sold to Seller', 'In Transit', 'In Stock', 'Sold to Consumer'];
      const timelineArr = history.map(h => ({
        status:   statusMap[Number(h.status)] || 'Unknown',
        location: h.location,
        notes:    h.notes,
      ...
      generateInvoicePDF(
        { serialNumber: serialNumber.trim(), name: product.name, description: product.description, brand: product.brand, timelineArr },
        { code: consumerCode.trim() }
      );

      setSerialNumber(''); setConsumerCode('');
    } catch (err) {
      console.error(err);
      setTxStatus('error');
  ...
  return (
    <div className="sell-product-consumer-container" style={{ position: 'relative', minHeight: '100vh' }}>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.5rem 2rem 0' }}>
        <button type="button" onClick={() => { window.location.hash = '/seller'; }}
          style={{ background: 'linear-gradient(90deg,#101830 0%,#00aaff 100%)', color: '#fff', border: 'none', borderRadius: 7, padding: '0.6rem 1.8rem', fontWeight: 700, cursor: 'pointer' }}>
          ← Back
        </button>
        <WalletConnect />
      </div>

      <h2 className="sell-product-consumer-title">Sell Product to Consumer</h2>
      ...
      <div className="qr-scanner-wrapper" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
        <div className="qr-camera-box" style={{ position: 'relative', minHeight: 240, width: 260, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', background: scannedQrImage ? `url(${scannedQrImage}) center/cover no-repeat` : '#101830', border: '2px solid #00aaff', borderRadius: 16 }}>
          {showScanner ? (
            <>
              <div ref={scannerRef} id="qr-html5-scanner-consumer" style={{ width: 240, height: 180, margin: '0 auto', zIndex: 2 }} />
          ...
          <div id="temp-upload-qr-div-consumer" style={{ display: 'none' }} />
        </div>
      </div>

      <form className="sell-product-consumer-form" onSubmit={handleSell}>
        <input type="text" placeholder="Product Serial Number" value={serialNumber} onChange={e => setSerialNumber(e.target.value)} />
        <input type="text" placeholder="Consumer Code" value={consumerCode} onChange={e => setConsumerCode(e.target.value)} />
        ...
        <button type="submit" disabled={txStatus === 'pending'} style={{ opacity: txStatus === 'pending' ? 0.6 : 1, cursor: txStatus === 'pending' ? 'not-allowed' : 'pointer' }}>
          {txStatus === 'pending' ? '⏳ Confirming on Blockchain...' : 'Sell Product'}
        </button>
      </form>

      {txStatus === 'success' && (
        <div style={{ margin: '16px 2rem', padding: '12px 16px', background: 'rgba(34,197,94,0.12)', border: '1px solid #22c55e', borderRadius: 8, color: '#22c55e', textAlign: 'center' }}>
